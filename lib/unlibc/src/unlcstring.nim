proc unlcstrlen(p: ptr cchar): csize_t {.exportc: "strlen".} =
  var i: uint = 0
...
proc unlcstrcmp(s1, s2: ptr cchar): cint {.exportc: "strcmp".} =
  var i = 0
...
proc unlcmemchr(blk: pointer, c: cint, size: csize_t): pointer {.exportc: "memchr".} =
...
proc unlcmemcpy(dst, src: pointer, size: csize_t): pointer {.exportc: "memcpy".} =
...
proc unlcmemset(dst: pointer, value: cint, num: csize_t): pointer {.exportc: "memset".} =
  if dst == nil:
    return
  result = memsetInternal(dst, value, num)

proc unlcmemcmp(a1, a2: pointer, size: csize_t): cint {.exportc: "memcmp".} =
  let
